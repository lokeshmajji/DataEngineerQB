...
object SparkJoinFiles extends App {
  ...
  val resultsql = spark.sql("select e.empid,e.empfname,e.emplname,e.salary,e.companyid,c.companyname,e.city,e.state from employee e join company c on e.companyid=c.companyid where e.state='Minnesota' and e.salary > 50000")
  println("Show Results using Spark SQL")
  resultsql.show()

  //using Spark DF
  val joineddf = empdf.join(companydf,empdf.col("companyid") === companydf.col("companyid"), "inner")
  val resultdf = joineddf.filter("state='Minnesota' and salary > 50000")
  ...
  resultsql.write.format("csv").option("sep","\t").mode(SaveMode.Append).save("C:\\Users\\LokeshMajji\\Desktop\\Loki\\Spark2\\out\\tsv")
